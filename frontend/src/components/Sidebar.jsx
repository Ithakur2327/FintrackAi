...
const DesktopSidebar = ({ children }) => {
  ...
  return (
    <motion.div
      className="h-full px-3 py-4 hidden md:flex md:flex-col bg-neutral-950 border-r border-neutral-900 shrink-0"
...
const MobileSidebar = ({ children }) => {
  ...
  return (
    <>
      <div className="h-14 px-4 flex md:hidden items-center justify-between bg-neutral-950 border-b border-neutral-900 w-full">
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            className="fixed h-full w-64 inset-0 bg-neutral-950 p-5 z-[100] flex flex-col border-r border-neutral-900"
          >
    ...
    >
      <Icon
        ...
        className={`shrink-0 transition-colors ${
          isActive ? "text-neutral-900 dark:text-white" : "text-neutral-500 group-hover:text-neutral-200"
        }`}
      />
      ...
      <motion.span
        ...
        className="text-sm font-medium whitespace-pre tracking-tight flex-1"
      >
        {item.label}
      </motion.span>
      ...
      {item.badge && showLabel && (
        <motion.span
          ...
          className="text-[10px] font-bold bg-neutral-700 text-neutral-300 px-1.5 py-0.5 rounded-md"
        >
          {item.badge}
        </motion.span>
      )}
...
const SidebarContent = ({ mobile = false, onClose }) => {
  ...
  return (
    <div className="flex flex-col h-full gap-1.5">
      {/* Logo — just the mark + name, no user pill below */}
      <div className={`flex items-center gap-2.5 px-3 py-3 mb-3 ${!showLabels && !mobile ? "justify-center" : ""}`}>
        ...
        <motion.div
          ...
          transition={{ duration: 0.1 }}
        >
          <p className="font-bold text-white text-sm tracking-tight">Fintrack</p>
        </motion.div>
      </div>
      ...
      {/* Bottom actions */}
      <div className="space-y-0.5 border-t border-neutral-900 pt-3 mt-1">
        <button
          onClick={() => setIsDark(p => !p)}
          className={`sidebar-link sidebar-link-inactive w-full ${!showLabels && !mobile ? "justify-center" : ""}`}
        >
          {isDark
            ? <Sun size={16} className="shrink-0 text-neutral-400" />
            : <Moon size={16} className="shrink-0 text-neutral-400" />
          }
          <motion.span
        ...
        <button
          onClick={logout}
          className={`sidebar-link w-full text-neutral-500 hover:bg-red-500/10 hover:text-red-400 transition-all duration-150 ${!showLabels && !mobile ? "justify-center" : ""}`}
        >
