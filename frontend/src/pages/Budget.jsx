      ...
      <motion.div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4"
        ...
        <motion.div className="bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 rounded-2xl w-full max-w-sm shadow-2xl p-6"
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              <label className="text-sm font-medium text-neutral-700 dark:text-neutral-300 block mb-1.5">Monthly Limit (₹)</label>
            ...
            <div>
              <label className="text-sm font-medium text-neutral-700 dark:text-neutral-300 block mb-2">
                Alert at <span className="text-orange-500 font-bold">{form.alertAt}%</span> usage
              </label>
    ...
    <div className="space-y-5 animate-fade-in">
      ...
      <div className="grid grid-cols-3 gap-2 sm:gap-4">
        {[
          ...
          { label: "Spent",        value: `₹${totalSpent.toLocaleString("en-IN")}`,              cls: "text-orange-500" },
          { label: "Remaining",    value: `₹${Math.abs(totalBudget - totalSpent).toLocaleString("en-IN")}`, cls: totalBudget - totalSpent >= 0 ? "text-emerald-600 dark:text-emerald-400" : "text-red-500" },
        ].map(s => (
              ...
              return (
                <motion.div key={budget._id} layout initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
                  ...
                  <div className="flex justify-between text-xs">
                    <span className="text-neutral-400">Limit: ₹{budget.limit.toLocaleString("en-IN")}</span>
                    <span className={isOver ? "text-red-500 font-semibold" : "text-emerald-600 dark:text-emerald-400 font-semibold"}>
                      {isOver ? `Over ₹${(budget.spent - budget.limit).toLocaleString("en-IN")}` : `₹${budget.remaining.toLocaleString("en-IN")} left`}
                    </span>
                  </div>
                </motion.div>
              );
