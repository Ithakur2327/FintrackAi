  ...
  return (
    <div className="space-y-4 sm:space-y-5 animate-fade-in pb-4">
      ...
      <div className="flex items-center justify-between gap-3 flex-wrap">
        <h1 className="text-xl sm:text-2xl font-black tracking-tight text-neutral-900 dark:text-neutral-50">AI Insights</h1>
      ...
      {/* Tabs */}
      <div className="flex gap-1 bg-neutral-100 dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 rounded-xl p-1 w-fit max-w-full">
        {[
          ...
          {loading && (
            <motion.div key="loading" initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}}
              ...
              <div className="text-center">
                ...
                <p className="text-sm text-neutral-400 mt-1">AI reviewing your transactions</p>
              </div>
      ...
      {/* ── Chat Tab ── */}
      {activeTab === "chat" && (
        <div className="card flex flex-col" style={{ height: "calc(100vh - 220px)", minHeight: "400px", maxHeight: "700px" }}>
          {/* Chat header */}
          <div className="flex items-center gap-3 pb-3 mb-3 border-b border-neutral-100 dark:border-neutral-800 shrink-0">
            <div className="w-8 h-8 sm:w-9 sm:h-9 rounded-xl bg-neutral-950 border border-neutral-800 flex items-center justify-center">
              <span className="text-[10px] sm:text-xs font-black text-white tracking-tight">AI</span>
            </div>
            <div>
              <p className="font-bold text-neutral-900 dark:text-neutral-100 text-sm">Fintrack Assistant</p>
              <div className="flex items-center gap-1.5 mt-0.5">
                <span className="w-1.5 h-1.5 rounded-full bg-emerald-500 inline-block" />
                <p className="text-xs text-neutral-400 font-medium">Online · AI Powered</p>
              </div>
            </div>
          </div>

          {/* Messages */}
          <div className="flex-1 space-y-3 overflow-y-auto mb-3 pr-1 no-visible-scrollbar">
            {chatMessages.map((msg,i) => (
              <motion.div key={i} initial={{opacity:0,y:5}} animate={{opacity:1,y:0}}
                ...
                <div className={`w-6 h-6 sm:w-7 sm:h-7 rounded-full flex items-center justify-center shrink-0 mt-0.5 ${
                  ...
                  {msg.role==="ai"
                    ? <span className="text-[8px] sm:text-[9px] font-black text-white">AI</span>
                    : <User2 size={11} className="text-neutral-600 dark:text-neutral-400" />
                  }
                </div>
                {/* Bubble */}
                <div className={`max-w-[82%] px-3 sm:px-4 py-2.5 sm:py-3 rounded-2xl text-xs sm:text-sm leading-relaxed ${
                  msg.role==="ai"
